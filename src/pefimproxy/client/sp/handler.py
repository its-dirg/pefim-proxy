        ...
        if session[self.SPHANDLERVERIFYTYPE] == "OP":
            ...
            if self.sp_conf.COPY_ASSERTION:
                try:
                    ...
                    if type(saml_response) is list and len(saml_response) > 0:
                        saml_response = saml_response[0]

                    ava = None
                    try:
                        authn_response = \
                        self.sp.parse_authn_request_response(saml_response,
                                                             binding,
                    ...
                    if ava is not None:
                        for k in ava:
                            if k not in sp_handler_cache.attributes:
                                sp_handler_cache.attributes[k] = ava[k]
