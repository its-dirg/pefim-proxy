...
class ForcedPersistentTestCase(helper.CPWebCase):
    BASEDIR = os.path.abspath(os.path.dirname(__file__))
    ARGS = Namespace(debug=False,
                     entityid="http://test.idp.se:1111/TestIdP.xml",
                     config="pefim_proxy_conf_force",
    ...
    def application(environ, start_response):
        return ForcedPersistentTestCase.WSGI_APP.run_server(environ, start_response)

    def setup_server():
        cherrypy.tree.graft(SessionMiddleware(ForcedPersistentTestCase.application, pefim_server_conf_default.SESSION_OPTS),
                            '/')
        pass
    ...
    def test_transient(self):
        ...
        self.assertTrue(tid2_1 == tid2_2, "Transient SP should get the same tid2 when the proxy is in the mode to force"
                                          "persistent nameid.")
    ...
    def test_persistent(self):
        ...
        self.assertTrue(tid2_1 == tid2_2, "Persistent SP should always get the same tid2 everytime.")
